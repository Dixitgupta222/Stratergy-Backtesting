...
export class CTraderMarket {
    connection;
    accountId;
    activeSpotSubscriptions = new Set();
    activeTrendbarSubscriptions = new Map();
    activeDepthSubscriptions = new Set();
    ...
    async subscribeSpots(symbolIds, subscribeToSpotTimestamp) {
        // Filter out symbols we already track -- the cTrader API throws
        // ALREADY_SUBSCRIBED on duplicates, which kills watchState internally.
        // Making this idempotent lets watchSpots + watchState overlap safely.
        const newIds = symbolIds.filter((id) => !this.activeSpotSubscriptions.has(id));
        if (newIds.length === 0)
            return;
        const payload = {
            ctidTraderAccountId: this.accountId,
