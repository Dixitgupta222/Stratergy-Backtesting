...
export function hostForEnvironment(environment) {
    return environment === "live" ? LIVE_HOST : DEMO_HOST;
}
/** @deprecated Use hostForEnvironment instead */
export function endpointForEnvironment(environment) {
...
function fromEnv() {
    ...
    if (env.CTRADER_CLIENT_ID)
        result.clientId = env.CTRADER_CLIENT_ID;
    ...
    if (env.CTRADER_ENVIRONMENT === "live" || env.CTRADER_ENVIRONMENT === "demo") {
...
export function resolveConfig(overrides) {
    ...
    const env = fromEnv();
    const merged = { ...stored, ...env, ...overrides };
    if (!merged.clientId)
        throw new Error("Missing clientId. Run `ctrader-ts auth` or set CTRADER_CLIENT_ID.");
    if (!merged.clientSecret)
        throw new Error("Missing clientSecret. Run `ctrader-ts auth` or set CTRADER_CLIENT_SECRET.");
    if (!merged.accessToken)
        throw new Error("Missing accessToken. Run `ctrader-ts auth` or set CTRADER_ACCESS_TOKEN.");
    if (!merged.accountId)
        throw new Error("Missing accountId. Run `ctrader-ts auth` or set CTRADER_ACCOUNT_ID.");
    const result = {
    ...
    if (merged.refreshToken)
        result.refreshToken = merged.refreshToken;
    return result;
}
