 ...
 */
export async function resolveSlTp(ctx, sl, tp) {
    const result = {};
...
async function specToPips(cache, spec, volumeInUnits) {
    if ("pips" in spec)
        return spec.pips;
    const sym = await cache.getSymbolDetails();
...
class RequestCache {
    ...
    fetchOneSpot() {
        return new Promise((resolve, reject) => {
            ...
            this.ctx.market.subscribeSpots([this.ctx.symbolId]).catch((err) => {
                clearTimeout(timeout);
                unsub();
                reject(err);
            });
