 ...
 */
function parseSlTp(raw) {
    const s = raw.trim();
    if (s.endsWith("%")) {
        const n = Number(s.slice(0, -1));
        if (Number.isNaN(n))
            throw new Error(`Invalid equity spec: ${raw}`);
        return { equity: n / 100 };
    }
    if (s.endsWith("e")) {
        const n = Number(s.slice(0, -1));
    ...
    if (s.startsWith("$")) {
        ...
        if (Number.isNaN(n))
            throw new Error(`Invalid dollar spec: ${raw}`);
        return { dollars: n };
    }
    if (s.endsWith("d")) {
        const n = Number(s.slice(0, -1));
    ...
    if (s.endsWith("p")) {
        const n = Number(s.slice(0, -1));
        if (Number.isNaN(n))
            throw new Error(`Invalid pips spec: ${raw}`);
        return { pips: n };
    }
    const n = Number(s);
    if (Number.isNaN(n))
        throw new Error(`Invalid SL/TP spec: ${raw}`);
    return { pips: n };
}
function buildMarketOpts(opts) {
    const result = { lots: Number(opts.lots) };
    if (opts.sl !== undefined)
        result.sl = parseSlTp(opts.sl);
    if (opts.tp !== undefined)
        result.tp = parseSlTp(opts.tp);
...
program
    ...
    .action(async (symbol, lots, limitPrice, opts) => {
    const ct = await getClient();
    ...
    if (opts.expires !== undefined)
        orderOpts.expirationTimestamp = Number(opts.expires);
...
program
    ...
    .option("--label <label>", "Order label")
    .option("--expires <ms>", "Expiry Unix ms timestamp")
    .action(async (symbol, lots, stopPrice, opts) => {
    const ct = await getClient();
    const base = buildMarketOpts({
        lots,
        sl: opts.sl,
...
program
    ...
    const modOpts = {};
    if (opts.sl !== undefined)
        modOpts.sl = parseSlTp(opts.sl);
    if (opts.tp !== undefined)
        modOpts.tp = parseSlTp(opts.tp);
...
program
    ...
    .action(async (positionId, newLots) => {
    const ct = await getClient();
...
program
    ...
    if (opts.from !== undefined)
        dealOpts.from = Number(opts.from);
    if (opts.to !== undefined)
        dealOpts.to = Number(opts.to);
    if (opts.max !== undefined)
        dealOpts.maxRows = Number(opts.max);
...
program
    ...
    .action(async (symbol, periodStr, from, to, opts) => {
    ...
    if (opts.count !== undefined)
        params.count = Number(opts.count);
...
program
    ...
    if (opts.json) {
        ...
        const rows = symbols.map((s) => [String(s.symbolId), s.symbolName || ""]);
        ...
        const sep = (widths) => `  ${widths.map((w) => "-".repeat(w)).join("  ")}\n`;
        const row = (cells, widths) => `  ${cells.map((c, i) => String(c).padEnd(widths[i] ?? 0)).join("  ")}\n`;
