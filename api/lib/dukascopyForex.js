...
/** Backtest window — 4 months for HTF; 1m uses 90 days (3 months, ~92k bars) */
const BACKTEST_DAYS = 124
const FOREX_1M_BACKTEST_DAYS = 90
...
function trimToDays(candles, days, intervalKey) {
...
async function fetchDukascopyCandles(symbol, intervalKey = '15m', opts = {}) {
  ...
  const cacheKey = `${instrument}_${intervalKey}_${requestedDays}`
  ...
  const to = new Date()
  const from = new Date(Date.now() - requestedDays * 86400 * 1000)
  ...
  let candles = trimToDays(sortUnique(all), requestedDays, intervalKey)
  ...
  if (requestedDays >= maxDays) {
    historyCache.set(`${instrument}_${intervalKey}_${maxDays}`, { at: Date.now(), data: candles })
  }
