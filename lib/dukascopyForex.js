...
async function fetchRange(instrument, timeframe, from, to) {
  ...
  return getHistoricalRates({
    ...
    volumes: true,
    ignoreFlats: true,
    // Vercel/Lambda cwd is read-only — disk cache throws and breaks all forex/metal history
    useCache: !serverless,
    cacheFolderPath: serverless ? '/tmp/.dukascopy-cache' : undefined,
    batchSize: serverless ? 8 : 12,
    pauseBetweenBatchesMs: serverless ? 80 : 150
  })
}
