/** Major, cross, exotic FX pairs + metals — Yahoo format: EURUSD=X */
const FOREX_UNIVERSE = [
  ...
  { symbol: 'XAUUSD', name: 'Gold / USD (Pepperstone spot)', base: 'XAU', quote: 'USD', type: 'metal' },
...
function toYahooSymbol(symbol) {
  ...
  if (YAHOO_METAL_MAP[s]) return YAHOO_METAL_MAP[s]
  return `${s}=X`
}
...
function searchSymbols(query = '', limit = 50) {
  ...
  if (!q) {
    const order = { major: 0, metal: 1, cross: 2, exotic: 3 }
    return [...FOREX_UNIVERSE]
      .sort((a, b) => (order[a.type] ?? 9) - (order[b.type] ?? 9))
      .slice(0, cap)
  }

  const results = FOREX_UNIVERSE.filter((item) => {
    const sym = item.symbol || ''
    const name = (item.name || '').toUpperCase()
    const base = (item.base || '').toUpperCase()
    const quote = (item.quote || '').toUpperCase()
    return (
      sym.includes(q) ||
      sym.startsWith(q) ||
      name.includes(q) ||
